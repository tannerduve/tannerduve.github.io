...
It may seem like there's an issue here. If I'm working in a total proof assistant, how do I talk about programs which may not terminate? Lean's Mathlib offers an elegant way to do this, and its theory is rooted in Dana Scott's *domain theory*, a subject which brings together order theory and topology to give mathematical semantics to programming languages. Given a type of values `α`, there is a type of *partial* values `Part α`, which captures nontermination via the undecidability of the halting problem. `Part` is in fact a monad in an interesting way, which allows us to sequence together partial computations such that nontermination propagates.
