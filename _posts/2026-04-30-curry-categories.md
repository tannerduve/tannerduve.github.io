...
This extends to the other connectives in basic logic: "and" corresponds to the cartesian product of types, "or" to their disjoint sum, "true" to the unit type, and "false" to the void/empty type. This can be made formal; taking a look at the proof rules for intuitionistic logic and the typing rules for the simply typed lambda calculus, up to some notational differences, we are really looking at the same thing:
...
We can view this as a functor: denote by $- \times B : \mathrm{C} \to \mathrm{C}$ the functor which sends an object $A$ to $A \times B$. We say a category is *cartesian closed* if the following hold:
