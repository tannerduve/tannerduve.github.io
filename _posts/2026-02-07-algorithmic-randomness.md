...
$$C_M(\sigma) = \min \{ |\tau| : M(\tau) = \sigma \}$$ where $\|\tau\|$ denotes the length of $\tau$ and $M(\tau) = \sigma$ means executing machine $M$ on $\tau$ halts with output $\sigma$.
...
$$U(\rho_M \sigma) = M(\sigma)$$ where $\rho_M$ is the coding string for machine $M$ and $\|\rho_M\|$ is the coding constant for $U$, the cost of simulating $M$. Fortunately for us, universal prefix-free machines exist and we can define prefix-free Kolmogorov complexity to be $K(\sigma) = C_U(\sigma)$ where $U$ is prefix-free. In our formalization, we have the following
