...
The free object which is most familiar to the programmer is the free monoid on a datatype `\alpha` — aka the type `List α`. This is of course the type of all finite sequences of elements of `\alpha`, which forms the monoid `(List \alpha, ++, [])`, where `++` is list concatenation.
